...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    @Transactional
    public AuthVO registerUser(RegisterDTO registerDTO) {
        try {
            // 验证密码确认
            ...
            // 根据用户类型创建对应的详细信息记录
            if ("student".equals(registerDTO.getUserType().name())) {
                ...
                // 处理学生感兴趣的科目关联
                if (registerDTO.getStudentSubjectIds() != null && !registerDTO.getStudentSubjectIds().isEmpty()) {
                    for (Long subjectId : registerDTO.getStudentSubjectIds()) {
                        StudentSubject studentSubject = new StudentSubject(student.getId(), subjectId);
                        studentSubjectMapper.insert(studentSubject);
                    }
                    log.info("学生科目关联创建成功: 学生ID={}, 科目数量={}", student.getId(), registerDTO.getStudentSubjectIds().size());
                }
            ...
            return new AuthVO(jwt, user.getId(), user.getUsername(),
     ...
     */
    @Override
    public AuthVO authenticateUser(LoginDTO loginDTO) {
        try {
            ...
            if (user == null) {
                ...
                String errorMessage = getLoginErrorMessage(loginDTO.getUsername());
            ...
            if (!"active".equals(user.getStatus())) {
                log.warn("登录失败，用户账户未激活: {}", loginDTO.getUsername());
     ...
     */
    @Override
    public AuthVO authenticateAdmin(LoginDTO loginDTO) {
        try {
            log.info("尝试管理员登录: {}", loginDTO.getUsername());
            ...
            if (!"admin".equals(user.getUserType())) {
                log.warn("登录失败，用户类型不是管理员: {}", loginDTO.getUsername());
                String errorMessage = getLoginErrorMessage(loginDTO.getUsername());
                throw new BadCredentialsException(errorMessage);
            }
            ...
            // 检查用户状态
            if (!"active".equals(user.getStatus())) {
                log.warn("管理员登录失败，账户未激活: {}", loginDTO.getUsername());
                throw new BadCredentialsException("账户未激活");
            }
    ...
    @Override
    @Transactional
    public boolean updateUserProfile(Long userId, UserUpdateDTO request) {
        try {
            User user = userMapper.selectById(userId);
            if (user == null) {
                log.warn("用户不存在: userId={}", userId);
