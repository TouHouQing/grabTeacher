...
public class StudentController {
     ...
     */
    @GetMapping("/profile")
    public ResponseEntity<ApiResponseDTO<StudentProfileResponseDTO>> getProfile(Authentication authentication) {
        try {
            UserPrincipal userPrincipal = (UserPrincipal) authentication.getPrincipal();
            StudentProfileResponseDTO studentProfile = studentService.getStudentProfileByUserId(userPrincipal.getId());

            if (studentProfile == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ApiResponseDTO.error("学生信息不存在"));
            }

            return ResponseEntity.ok(ApiResponseDTO.success("获取成功", studentProfile));
        } catch (Exception e) {
     ...
     */
    @PutMapping("/profile")
    public ResponseEntity<ApiResponseDTO<Student>> updateProfile(
            @Valid @RequestBody StudentInfoRequestDTO request,
            Authentication authentication) {
        try {
            UserPrincipal userPrincipal = (UserPrincipal) authentication.getPrincipal();
            Student updatedStudent = studentService.updateStudentInfo(userPrincipal.getId(), request);

            return ResponseEntity.ok(ApiResponseDTO.success("更新成功", updatedStudent));
