...
public class JobPostCacheManager {
    ...
    public String buildListKey(int page, int size, Long subjectId) {
        String s = subjectId == null ? "ALL" : String.valueOf(subjectId);
        return LIST_PREFIX + "page_" + page + ":size_" + size + ":subject_" + s;
    }
    ...
    public String buildAdminListKey(int page, int size,
        ...
        return LIST_PREFIX + "admin:page_" + page + ":size_" + size + ":s_" + s + ":st_" + st + ":kw_" + kw + ":cs_" + cs + ":ce_" + ce + ":del_" + del;
    }

    public void saveList(String key, Object pageObj, Long subjectId, Duration ttl) {
    ...
    public void evictByDimensions(Set<Long> subjectIds) {
        // 收集需要删除的缓存键
