...
import { teachingLocationAPI } from '../../../utils/api'
...
const loadList = async () => {
  try {
    ...
    const res = await teachingLocationAPI.getList(params)
    if (res?.success && res.data) {
      const data = res.data
      list.value = Array.isArray(data) ? data : (data.locations || [])
      pagination.total = data.total ?? list.value.length
    }
  } catch (e: any) {
    console.error('获取授课地点失败:', e)
...
// 新增
const onAdd = () => {
  dialogTitle.value = '添加授课地点'
  Object.assign(form, { id: 0, name: '', address: '', isActive: true, sortOrder: 0 })
  dialogVisible.value = true
}

// 编辑
const onEdit = (row: any) => {
  dialogTitle.value = '编辑授课地点'
...
const onSave = async () => {
  try {
    loading.value = true
    const payload = { name: form.name, address: form.address, isActive: form.isActive, sortOrder: form.sortOrder }
    const res = form.id === 0
      ? await teachingLocationAPI.create(payload)
      : await teachingLocationAPI.update(form.id, payload)
    if (res?.success) {
...
const onDelete = async (row: any) => {
  try {
    ...
    const res = await teachingLocationAPI.delete(row.id)
    if (res?.success) {
...
const onToggleStatus = async (row: any) => {
  try {
    const res = await teachingLocationAPI.updateStatus(row.id, !row.isActive)
    if (res?.success) {
    ...
    <el-card class="search-card" shadow="never">
      <el-form :model="searchForm" inline>
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="搜索名称/地址" :prefix-icon="Search" clearable style="width: 240px" />
        </el-form-item>
        ...
        <el-form-item>
          ...
          <el-button type="primary" :icon="Plus" @click="onAdd">新增地点</el-button>
        </el-form-item>
      </el-form>
    </el-card>
...
<style scoped>
.location-management { padding: 20px; padding-top: 5px; }
