...
const form = reactive({ id: 0, stageName: '', sortOrder: 0, isActive: true })
...
const onAdd = () => {
  ...
  Object.assign(form, { id: 0, stageName: '', sortOrder: 0, isActive: true })
...
const onSave = async () => {
  try {
    loading.value = true
    const payload = { stageName: form.stageName, sortOrder: form.sortOrder, isActive: form.isActive }
    const res = form.id === 0
...
const onToggleStatus = async (row: any) => {
  try {
    const res = await studyAbroadAPI.adminUpdateStageStatus(row.id, !row.isActive)
